...
export function MiniAdminInspectionsOverview({ onUpdate }: InspectionsOverviewProps) {
  ...
  const fetchInspections = async () => {
    try {
      const response = await fetch("/api/mini-admin/inspections")
      if (response.ok) {
  ...
  const downloadPDF = async (inspectionId: string, templateName: string) => {
    ...
    try {
      const response = await fetch(`/api/mini-admin/inspections/${inspectionId}/generate-pdf`)
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Recent inspections in your area</CardDescription>
        </CardHeader>
        <CardContent>
